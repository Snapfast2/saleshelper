  ...
  return (
    <div style={{ padding: "20px 16px 120px", maxWidth: 520, margin: "0 auto", fontFamily: "Inter, sans-serif" }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 23, fontWeight: 800, color: "var(--text-primary)", margin: 0 }}>
          🔍 Diagnóstico
        </h1>
        ...
        {data?.esDomingo && (
          <div style={{ marginTop: 8, padding: "8px 12px", borderRadius: 10, background: "rgba(251,191,36,0.15)", border: "1px solid rgba(251,191,36,0.4)", fontSize: 14, color: "#fbbf24" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Section title="👥 Prueba de clientes en vivo">
            {data.fetchClientes?.error ? (
              <div style={{ color: "#f87171", fontSize: 14, wordBreak: "break-word" }}>
              ...
              <>
                ...
                {data.fetchClientes?.nuevos?.ok && data.fetchClientes.nuevos.cantidad === 0 && (
                  <div style={{ fontSize: 13, color: "#fbbf24", marginTop: 4, padding: "6px 10px", background: "rgba(251,191,36,0.1)", borderRadius: 8 }}>
          ...
          <Section title="">
            <div style={{ textAlign: "center", padding: "8px 0" }}>
              <div style={{ fontSize: 41 }}>{verdict.icon}</div>
              <div style={{ fontSize: 17, fontWeight: 700, color: verdict.color, marginTop: 6 }}>{verdict.label}</div>
              <div style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 4 }}>{verdict.sub}</div>
            </div>
          </Section>
          ...
          <Section title="🎭 Headers exactos que enviamos">
            ...
            <div style={{ marginTop: 8, fontSize: 13, color: "var(--text-muted)", background: "rgba(0,0,0,0.2)", borderRadius: 8, padding: 10 }}>
          ...
          <Section title="💾 Sesión en Redis">
            {data.sesion?.error ? (
              <div style={{ color: "#f87171", fontSize: 14 }}>{data.sesion.error}</div>
            ) : (
              <>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{
      ...
      {title && <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-secondary)", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.05em" }}>{title}</div>}
...
function Row({ label, value, mono, small, badge }: { label: string; value: string; mono?: boolean; small?: boolean; badge?: string }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
      <span style={{ fontSize: 14, color: "var(--text-secondary)", flexShrink: 0, paddingTop: 1 }}>{label}</span>
