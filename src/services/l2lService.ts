...
const DEFAULT_L2L_URL = "https://www.l2lbienesraices.com/busqueda/pagina/1/agente/29214";

export async function scrapeL2L(): Promise<{ inmuebles: Inmueble[], fuente: string, total: number }> {
  try {
    const url = process.env.L2L_PROFILE_URL || DEFAULT_L2L_URL;
    ...
    if (!res.ok) {
      throw new Error(`L2L respondió con status ${res.status}`);
    }
    ...
    // Parsear cada propiedad del grid
    $(".property-box2").each((_, el) => {
      ...
      const gestion: "venta" | "arriendo" =
        gestionText === "arriendo" ? "arriendo" : "venta";
      ...
      // Ciudad
      const ciudad = $el.find(".location-area")
        .text()
    ...
    return { inmuebles, fuente: "l2l", total: inmuebles.length };
  } catch (error) {
